...
The models are large files, so they use git LFS. Make sure to clone with git LFS or you will only get pointers to these files, not the files themselves. Additionally, the model files are compressed .tgz files. Please decompress the model file, which will result in a directory containing all necessary files. 
